// ============================================================================
// Buffer A —— 场景渲染 pass（不直接显示，输出给 Image pass 做后处理）
//
// 原作：Up in the Cloud Sea by mdb
// 来源：https://www.shadertoy.com/view/Ndc3zl
// 授权：Shadertoy 默认 CC BY-NC-SA 3.0（非商用 + 署名 + 相同方式共享）
//
// 运行时通道绑定（见 HeroBanner.vue 中的 bufferChannels 配置）：
//   - iChannel0 = 蓝噪声贴图 /textures/blue_noise.png（noise() 函数的采样源）
//   - iChannel1 = Buffer A 自身上一帧（30% 反馈混合，形成手绘拖影质感）
// ============================================================================
float noise(vec2 x){
    vec2 f = fract(x);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //uv.x += iTime;
    // 站点适配：原作为 iTime*4.0，整体放慢流速（约 1/3），观感更沉静
    float t = iTime*1.2;
