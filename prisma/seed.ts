...
async function main() {
  ...
  // 3. 初始化标签 (Tags)
  const tagVue = await prisma.tag.upsert({ where: { slug: 'vue3' }, update: {}, create: { name: 'Vue 3', slug: 'vue3' } })
  const tagNuxt = await prisma.tag.upsert({ where: { slug: 'nuxt3' }, update: {}, create: { name: 'Nuxt 3', slug: 'nuxt3' } })
  const tagTailwind = await prisma.tag.upsert({ where: { slug: 'tailwindcss' }, update: {}, create: { name: 'Tailwind CSS', slug: 'tailwindcss' } })
  ...
  const tagArch = await prisma.tag.upsert({ where: { slug: 'architecture' }, update: {}, create: { name: '架构设计', slug: 'architecture' } })
  const tagUI = await prisma.tag.upsert({ where: { slug: 'ui-ux' }, update: {}, create: { name: 'UI/UX', slug: 'ui-ux' } })
  const tagPKM = await prisma.tag.upsert({ where: { slug: 'pkm' }, update: {}, create: { name: '知识管理', slug: 'pkm' } })
  ...
  const articlesData = [
    {
      ...
      summary: '聊聊我为什么要搭建「神秘花园」，以及我是如何用 Nuxt 3、Tailwind CSS 和 MySQL 构建这方兼顾技术深度与灵感美学的数字自留地。',
      content: `# 欢迎来到神秘花园
...
欢迎常来逛逛，在下方留言交流！`,
    ...
    },
    {
      ...
      summary: '剖析 Nuxt 3 服务端渲染（SSR）到客户端激活（Hydration）的全链路流程，彻底规避双重请求、内存泄漏与水合不匹配。',
...
- [x] 开启 Nitro 强缓存头部 \`Cache-Control: s-maxage=3600\`。`,
    ...
    },
    {
      ...
      summary: '从色彩体系、MD3 药丸交互、动态视差毛玻璃到暗黑模式平滑过渡，打造极具呼吸感与辨识度的现代博客 UI。',
...
- 动态 SVG 视差波浪将 Banner 大图与页面底色无缝衔接。`,
    ...
    },
    {
      ...
      summary: '如何利用纯 CSS 硬件加速与 SVG 视差分层，实现零卡顿、零 JS 算力开销的流体波浪自然交融效果。',
...
使用 \`translate3d\` 触发 GPU 复合图层渲染，帧率稳健保持在 60/120 FPS。`,
    ...
    },
    {
      ...
      summary: '探索 Const Type Parameters、Satisfies 运算符与 Zod 运行时 Schema 如何构建端到端零 any 的严谨类型体系。',
...
允许泛型参数直接推断为常量字面量类型，极大减少 \`as const\` 的样板代码。`,
      coverImage: 'https://images.unsplash.com/photo-1516116211227-bbc13c73335c?w=800&auto=format&fit=crop&q=80',
      isPinned: false,
      isPublished: true,
      views: 260,
      readingTime: 7,
    ...
    },
    {
      ...
      summary: '从收集、提炼到公开输出，建立一套不依赖庞杂插件、直面核心思考的极简个人知识闭环。',
...
> 输出是最高效的输入。当你要向别人讲清楚一门技术时，你才真正掌握了它。`,
    ...
    },
    {
      ...
      summary: '构建体积缩减 80%！手把手打造精简、安全且具备健康检查与 Prisma 迁移机制的生产级镜像。',
...
`,
    ...
    },
    {
      ...
      summary: '解决深色模式切换瞬间白屏闪烁、SVG 描边跳变，实现类似 iOS/macOS 级别的高质感全局色彩渐变。',
      content: `# 深色模式平滑切换实践
...
`,
    ...
    },
    {
      ...
      summary: '使用 CVA + clsx + tailwind-merge 构建强类型、可复用的 UI 基础组件',
...
`,
    ...
    },
    {
      ...
      summary: '针对大型不变数组或 Markdown AST 节点，使用浅层响应式避免深层代理开销',
...
`,
    ...
    },
    {
      ...
      summary: '解读 EXPLAIN 中的 type、key、rows 与 Extra 字段，规避 filesort 与全表扫描',
...
`,
  ...
  const essaysData = [
    {
      content: '完成了「神秘花园」的全站 Mizuki 风格动态导航与 4 层动态波浪改造。在深色模式下的 Midnight Indigo 与晴空蓝搭配极其协调，呼吸感拉满！',
      mood: '专注',
      weather: '晴朗',
      location: '工作室',
      likes: 18,
    ...
    {
      content: '重新审视了许多顶尖博客的设计，发现真正耐看的产品往往不是堆砌复杂的阴影和渐变，而是对字距、行高、留白以及动画曲线的极致克制。',
      mood: '思考',
      weather: '多云',
      location: '书房',
      likes: 14,
    ...
    {
      content: '下午泡了一杯乌龙茶，一口气把 Nuxt 3 的服务端中间件和路由转场梳理完毕。当代码像交响乐一样流畅运转时，那种愉悦感无与伦比。',
      mood: '闲适',
      weather: '微风',
      location: '阳台',
      likes: 9,
    ...
    {
      content: '读完了《代码整洁之道》，好代码就像一篇通俗易懂的散文，不需要复杂的注释就能让人一目了然。重构永远是开发中最享受的环节。',
      mood: '阅读',
      weather: '晴朗',
      location: '图书馆',
      likes: 12,
    ...
    {
      content: '今天给站点加入了全局顶部蓝色流光阅读进度条，跟随整页滚动平滑延展，阅读长文时的交互体验直线上升。',
      mood: '日常',
      weather: '晴天',
      location: '咖啡厅',
      likes: 7,
    ...
    {
      content: '折腾了一下午 Docker 多阶段构建，成功把生产镜像体积从 1.1GB 压缩到 130MB，部署启动耗时缩短了整整 70%！',
      mood: '专注',
      weather: '阵雨',
      location: '工位',
      likes: 21,
  ...
  console.log('🎉「神秘花园」全量深度文章 (8篇)、知识笔记 (6篇)、微言随笔 (6条) 初始化完毕！')
}
